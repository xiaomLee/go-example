...
func (u UserResource) WebService() *restful.WebService {
	...
	ws.Route(ws.GET("/watch").To(u.watchUsers).
		// docs
		Doc("watch all users").
		Metadata(restfulspec.KeyOpenAPITags, tags).
		Writes(User{}).
...
// GET http://localhost:8080/users/watch
//
func (u UserResource) watchUsers(request *restful.Request, response *restful.Response) {
...
func (u *UserResource) sendEvent(e Event) {
	select {
	case u.event<-e:
	default:
	}
...
func main() {
	u := UserResource{users: map[string]User{}, event: make(chan Event, 50)}
